$(document).ready(function(){
    ...
    $(document).on('click', '.eliminar_valoracion',function(e){
        e.preventDefault();
        var a = $(this);
        $.post(base_url+'mvc/controllers/CValoracionGeron.php?action=eliminarValoracionbraden',{id_resultado_indice:$(this).attr('id_resultado_indice'),id_cedula:$("#cedula").val()},
            function(respuesta){
    ...
    $(document).on('click','#guardar',function(e){
        if($('#frm_valoracion').valid()){
            ...
            $.post(base_url+'mvc/controllers/CValoracionGeron.php?action=guardarbraden', $('#frm_valoracion').serialize(),function(data){
                overlay.hide();
                if(data.rpt){
    ...
    function recargarItemsResultados() {
        $('#tbl_items_resultantes').load(base_url+'mvc/controllers/CValoracionGeron.php?action=listaIndicebraden',{cedula:$('#cedula').val()});
    }
    ...
    $(document).on('click','.imprimir_excel',function(e){
        window.open(
          base_url+'mvc/controllers/CValoracionGeron.php?action=exportarExcelIndicebraden&id_encabezado='+$(this).attr('id_encabezado'),
    ...
    $("#tipo_valoracion").select2({
        ...
        language: "es",
        ajax: { 
            url: base_url+'mvc/controllers/CValoracionGeron.php?action=obtenerTipobraden',
            cache: "true",
            type:'POST',
            dataType: 'json',
            data: function (data, page) {
    ...
    $("#indice_barthel").select2({
        ...
        language: "es",
        ajax: { 
            url: base_url+'mvc/controllers/CValoracionGeron.php?action=obtenerIndicebranden',
            cache: "true",
            type:'POST',
            dataType: 'json',
            data: function (data, page) {
