$(document).ready(function(){
	...
	$(document).on('click','#btn_editar_usuario',function(e){
		if($('#frm-roles').valid() && $('#frm-roles').valid()){
			var load = open_overlay();
			$.post(base_url+'mvc/controllers/CRoles.php?action=guardarEdicionUsuario',$("#frm-roles").serialize(),function(respuesta){
				if(respuesta.rpt){
					$.notify(respuesta.mensaje,"success");
		       	}else{
		       		$.notify(respuesta.mensaje,"error");
