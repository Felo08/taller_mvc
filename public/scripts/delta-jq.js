$(document).ready(function(){
    ...
    $(document).on('click','#guardar',function(e){
        if($('#frm_valoracion').valid()){
            ...
            $.post(base_url+'mvc/controllers/CValoracionGeron.php?action=guardardelta', $('#frm_valoracion').serialize(),function(data){
                overlay.hide();
                if(data.rpt){
    ...
    function recargarItemsResultados() {
        $('#tbl_items_resultantes').load(base_url+'mvc/controllers/CValoracionGeron.php?action=listadelta',{cedula:$('#cedula').val()});
    }
    ...
    $(document).on('click','.imprimir_excel',function(e){
        window.open(
          base_url+'mvc/controllers/CValoracionGeron.php?action=exportarExceldelta&id_encabezado='+$(this).attr('id_encabezado'),
    ...
    $(document).on('click','.continuar_indice',function(e){
        ...
        $("#id_principal").select2({
        placeholder: ("Pregunta delta"),
        allowClear: true,
        language: "es",
        ajax: { 
            url: base_url+'mvc/controllers/CValoracionGeron.php?action=obtenerdeltappal',
            cache: "true",
            type:'POST',
            dataType: 'json',
            data: function (data, page) {
 ...
 $("#tipo_valoracion").select2({
        ...
        language: "es",
        ajax: { 
            url: base_url+'mvc/controllers/CValoracionGeron.php?action=obtenerdeltaitem',
            cache: "true",
            type:'POST',
            dataType: 'json',
            data: function (data, page) {
                data['tipo_valoracion'] = $("#id_principal").val();
                data['cedula'] = $("#cedula").val();
                data['id_encabezado'] = $("#id_encabezado").val();
                return data;
            }
    ...
    $("#id_principal").change(function(e){
        data = $(this).select2("data");
        if(data[0] != undefined){
    ...
    $("#indice_barthel").select2({
        placeholder: "respuesta delta",
        allowClear: true,
        language: "es",
        ajax: { 
            url: base_url+'mvc/controllers/CValoracionGeron.php?action=obtenerdelta_item',
            cache: "true",
            type:'POST',
            dataType: 'json',
            data: function (data, page) {
