$(document).ready(function(){
	...
	$("#id_cedula").select2({
	    ...
	    language: "es",
	    ajax: { 
	        url: base_url+'mvc/controllers/CHuesped.php?action=obtenerHuespedingresomedicoSelectcom', 
	        cache: "true",
	        type:'POST',
	        dataType: 'json',
   ...
   console.log("saludo");
		if($('#frm-medica').valid() && $('#frm-medica').valid()){
			loader = open_overlay();
			$.post(base_url+'mvc/controllers/Cingresomedico.php',$("#frm-medica").serialize(),function(respuesta){
				if(respuesta.rpt){
	...
	$(document).on('keyup',"#buscar_usuario",function(){
      $.post(base_url+'mvc/controllers/Cingresomedico.php',$("#frm_buscar").serialize(),function(respuesta)
      {
