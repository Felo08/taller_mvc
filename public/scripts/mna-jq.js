$(document).ready(function(){
    ...
    $(document).on('click', '.eliminar_valoracion',function(e){
        e.preventDefault();
        var a = $(this);
        $.post(base_url+'mvc/controllers/Cmna.php?action=eliminarValoracionIndiceBarthel',{id_resultado_indice:$(this).attr('id_resultado_indice'),id_cedula:$("#cedula").val()},
            function(respuesta){
    ...
    $(document).on('click','#guardar',function(e){
        if($('#frm_valoracion').valid()){
            ...
            $.post(base_url+'mvc/controllers/Cmna.php?action=guardarIndiceBarthel', $('#frm_valoracion').serialize(),function(data){
                overlay.hide();
                if(data.rpt){
    ...
    function recargarItemsResultados() {
        $('#tbl_items_resultantes').load(base_url+'mvc/controllers/Cmna.php?action=listaIndiceBarthel',{cedula:$('#cedula').val()});
    }
    ...
    $(document).on('click','.imprimir_excel',function(e){
        window.open(
          base_url+'mvc/controllers/Cmna.php?action=exportarExcelIndiceBarthel&id_encabezado='+$(this).attr('id_encabezado'),
    ...
    $("#tipo_valoracion").select2({
        ...
        language: "es",
        ajax: { 
            url: base_url+'mvc/controllers/Cmna.php?action=obtenerTipoValoracion',
            cache: "true",
            type:'POST',
            dataType: 'json',
            data: function (data, page) {
    ...
    $("#indice_barthel").select2({
        ...
        language: "es",
        ajax: { 
            url: base_url+'mvc/controllers/Cmna.php?action=obtenerIndiceBarthel',
            cache: "true",
            type:'POST',
            dataType: 'json',
            data: function (data, page) {
    ...
    $("#talla").change(function(e){
        ...
        imc=Math.round(parseFloat(ld_peso)/(Math.pow(parseFloat(ld_talla),2)))
        $('#id_imc').html(imc);
        $('#id_imc').val(imc); 
