$(document).ready(function(){
	...
	$("#id_cedula").select2({
	    ...
	    language: "es",
	    ajax: { 
	        url: base_url+'mvc/controllers/CHuesped.php?action=obtenerHuespedsocialamanSelectcom',
	        cache: "true",
	        type:'POST',
	        dataType: 'json',
	...
	$("#bnt-guardar").click(function(e){
		...
		if($('#frm-medica').valid() && $('#frm-medica').valid()){
			loader = open_overlay();
			$.post(base_url+'mvc/controllers/Chpersonal.php',$("#frm-medica").serialize(),function(respuesta){
				if(respuesta.rpt){
