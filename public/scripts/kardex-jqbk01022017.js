$(document).on('ready',function(e){
		...
		$.post(base_url+'mvc/controllers/CKardex.php?action=cargarFrecuenciaDosisPaciente',{cedula:$("#cedula").val(),id_medicamento:$(this).attr('id_medicamento')},function(data){
			if(data.rpt){
	          	for (var i = 1; i <= 13; i++) {
	          		indexdia = 'dia'+i;
	          		indexnoche = 'dia'+i;
	          		$('input[name=dia'+i+']').val(data.dia[indexdia]);
	          		$('input[name=noche'+i+']').val(data.noche[indexnoche]);
	          	}
	...
	$(document).on('click','.suspender_medicamento', function(e){
		...
		bootbox.confirm({ 
		  size: "small",
		  message: "Esta seguro que desea suspender este medicamento?", 
		  callback: function(result){ 
		  	if(result){
		  		$.post(base_url+'mvc/controllers/CKardex.php?action=suspenderMedicamento',{cedula:$("#cedula").val(),id_medicamento:id_medicamento},function(respuesta){
		  			if(respuesta.rpt){
		  				recargarGridMedicamentosKardex();
			          	$.notify(respuesta.mensaje,'success');
			        }else{
