...
$("#responsable_factura").on('change',function(){
		if($(this).val() != null)
		{
		   if($(this).val() =='1')
		  {

            $('#id_nombre_factura').val("N/A");
            $('#id_nit_factura').val("N/A");
            $('#id_direccion_factura').val("N/A");
            $('#id_telefono_factura').val("N/A");
            $('#id_correo_factura').val("N/A");
		  }
		  else
		  {
           console.log(""); 
            
             $('#id_nombre_factura').val("");
		      $('#id_nit_factura').val("");
              $('#id_direccion_factura').val("");
              $('#id_telefono_factura').val("");
              $('#id_correo_factura').val("");   
	...
	$(document).on('click', ".btn_edit_product", function()
	{
			...
			$("#id_responsable_factura").val(respuesta.reponsable_facturacion);
			$("#nombre_factura").val(respuesta.nombre_factura);
			$("#nit_factura").val(respuesta.nit_factura);
			$("#telefono_factura").val(respuesta.telefono_factura);
			$("#direccion_factura").val(respuesta.direccion_factura);
			$("#correo_factura").val(respuesta.correo_factura);
			$("#id_parentesco").val(respuesta.parentesco);
			$("#id_telefono_re").val(respuesta.telefono_re);
			$("#id_domicilio").val(respuesta.domicilio);
