$(document).ready(function(){
	...
	$("#bnt-guardar").click(function(e){
		if($('#frm-medica').valid() && $('#frm-medica').valid()){
			loader = open_overlay();
			$.post(base_url+'mvc/controllers/Csalud.php',$("#frm-medica").serialize(),function(respuesta){
				if(respuesta.rpt){
  ...
  $(document).on('keyup', '#id_peso', function (e) {
		calcularValorIMC('');
	})

	$(document).on('keyup', '#id_talla', function (e) {
		calcularValorIMC('');
	});
	...
	$(document).on('click', '#btn_adicionar_hijo', function(e){
		...
		if($('#nombre_hijo').val().length>0){
			...
			html += '<td>'+$('#frecuencia').val()+'<input type="hidden" value="'+$('#frecuencia').val()+'" name="hijos[frecuencia][]"/></td>';
			html += '<td>'+$('#via').val()+'<input type="hidden" value="'+$('#via').val()+'" name="hijos[via][]"/></td>';
			html += '<td>'+$('#hora').val()+'<input type="hidden" value="'+$('#hora').val()+'" name="hijos[hora][]"/></td>';
...
function calcularValorIMC(id) {
	if ( 
		($('#id_peso'+id).val() != 'N/A' && !isNaN($('#id_peso'+id).val()) && $('#id_peso'+id).val().length > 0 ) &&
		($('#id_talla'+id).val() != 'N/A' && !isNaN($('#id_talla'+id).val()) && $('#id_talla'+id).val() != '0' && $('#id_talla'+id).val().length > 0) 
		) {
		IMC = parseFloat($('#id_peso'+id).val())/Math.pow($('#id_talla'+id).val(), 2);
		$('#id_imc'+id).val(Math.round(IMC));
	}else{
		$('#id_imc'+id).val('N/A');
	}
}
	...
	function recargar_grid(){
		 ...
		 $.post(base_url+'mvc/controllers/Csalud.php',{action:"recargar_salud"},function(respuesta)
	            {
	            	$("#contenedor_list_productos").html(respuesta);
	            });
	}