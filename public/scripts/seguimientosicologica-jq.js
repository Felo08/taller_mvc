...
$("#id_tipo_valoracion").select2({
	    placeholder: "Buscar tipo de valoracion",
	    allowClear: true,
	    language: "es",
	    ajax: { 
	        url: base_url+'mvc/controllers/CHuesped.php?action=obtenertipovaloracionSelect',
	        cache: "true",
	        type:'POST',
	        dataType: 'json',
	        data: function (data, page) {
   ...
   console.log("sssss");
		if($('#frm-medica').valid() && $('#frm-medica').valid()){
			loader = open_overlay();
			$.post(base_url+'mvc/controllers/Cnotas_fisioterapeuta.php',$("#frm-medica").serialize(),function(respuesta){
				if(respuesta.rpt){
	...
	function recargar_grid(){
		 ...
		 $.post(base_url+'mvc/controllers/Cnotas_fisioterapeuta.php',{action:"recargar_seguimientosicoogica"},function(respuesta)
	            {
	            	$("#contenedor_list_productos").html(respuesta);
	            });
	}