$(document).ready(function(){
	...
	$("#bnt-guardar").click(function(e){
		...
		if($('#frm-medica').valid() && $('#frm-medica').valid()){
			//loader = open_overlay();
			$.post(base_url+'mvc/controllers/Ccontrol_enfermeria.php?action=guardar_entregaitem',$("#frm-medica").serialize(),function(respuesta){
				if(respuesta.rpt){
					enviarNotificacion(respuesta.dataNotificacion.usuarioNotificado);
					idusuario = JSON.parse(sessionStorage.getItem('user')).userid;
					crearNotificacionHTML(respuesta.dataNotificacion.usuarioNotificado[idusuario]);
