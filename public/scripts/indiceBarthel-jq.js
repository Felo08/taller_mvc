$(document).ready(function(){
    ...
    $(document).on('click','#guardar',function(e){
        if($('#frm_valoracion').valid()){
            ...
            $.post(base_url+'mvc/controllers/CValoracionGeron.php?action=guardarIndiceBarthel', $('#frm_valoracion').serialize(),function(data){
                overlay.hide();
                if(data.rpt){
    ...
    function recargarItemsResultados() {
        $('#tbl_items_resultantes').load(base_url+'mvc/controllers/CValoracionGeron.php?action=listaIndiceBarthel',{cedula:$('#cedula').val()});
    }
    ...
    $(document).on('click','.imprimir_excel',function(e){
        window.open(
          base_url+'mvc/controllers/CValoracionGeron.php?action=exportarExcelIndiceBarthel&id_encabezado='+$(this).attr('id_encabezado'),
    ...
    $("#tipo_valoracion").select2({
        ...
        language: "es",
        ajax: { 
            url: base_url+'mvc/controllers/CValoracionGeron.php?action=obtenerTipoValoracion',
            cache: "true",
            type:'POST',
            dataType: 'json',
            data: function (data, page) {
    ...
    $("#indice_barthel").select2({
        placeholder: "Indice Barthel",
        allowClear: true,
        language: "es",
        ajax: { 
            url: base_url+'mvc/controllers/CValoracionGeron.php?action=obtenerIndiceBarthel',
            cache: "true",
            type:'POST',
            dataType: 'json',
            data: function (data, page) {
