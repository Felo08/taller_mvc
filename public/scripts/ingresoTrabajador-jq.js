$(document).ready(function(){
   ...
   console.log("saludo");
		if($('#frm-medica').valid() && $('#frm-medica').valid()){
			loader = open_overlay();
			$.post(base_url+'mvc/controllers/CingresoTrabajador.php',$("#frm-medica").serialize(),function(respuesta){
				if(respuesta.rpt){
	...
	$(document).on('keyup',"#buscar_usuario",function(){
      $.post(base_url+'mvc/controllers/CingresoTrabajador.php',$("#frm_buscar").serialize(),function(respuesta)
      {
	...
	function recargar_grid(){
		 ...
		 $.post(base_url+'mvc/controllers/CingresoTrabajador.php',{action:"recargar_ingreso"},function(respuesta)
	            {
	            	$("#contenedor_list_productos").html(respuesta);
	            });
	}